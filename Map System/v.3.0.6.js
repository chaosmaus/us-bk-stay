        ...
        $('.card-marker').each((i, el) => {
          if ($(element).find('.item_guesty-id').text() === $(el).find('.marker-guesty').text()) {
            ...
            if ($(element).find('.per-night').text() === `NaN`) {
              ...
              $(el).css(`background-image`, `url("https://uploads-ssl.webflow.com/643e7d5bd3fee28a34417207/65bfe4417c57fd5d85cea32b_us-icon.png")`);
      ...
      if (data.availability.length !== 0) {
        ...
        $(".item").each((index, element) => {
          ...
          availableProperties.forEach((arrEl, arrIndex) => {
            if (currentId === arrEl.listingId && guests <= Number(arrEl.accommodates)) {
        ...
        map.addLayer({
          ...
          paint: {
            "circle-color": [
              ...
              "#9FC8B3", // Color when hovered
              [
                "step",
                ["get", "point_count"],
                "#D1FAE5",  // Default color
                20,
                "#D1FAE5",
                50,
                "#D1FAE5",
            ...
            "circle-radius": [
              ...
              0, 20,   // 10px radius for 0 points (log10(1) = 0)
              2, 40    // 52px radius for 100 points (log10(100) = 2)
          ]
